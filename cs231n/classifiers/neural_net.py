...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #############################################################################
        S1 = X.dot(W1) + b1
        S1_relu = np.maximum(S1, 0)
        S2 = S1_relu.dot(W2) + b2
        scores = S2
        pass
        #############################################################################
        #                              END OF YOUR CODE                             #
        ...
        #numers = expscores[(range(N),y)]
        #loss = -1 * np.log((numers / denoms).sum) / N
        denoms = np.tile(denoms, (scores.shape[1],1)).T
        ...
        grads['W2'] = S1_relu.T.dot(dscores)
        grads['b2'] = dscores.sum(axis=0)
        
        dS1_relu = dscores.dot(W2.T)
        dS1_pos = np.zeros_like(S1)
        dS1_pos[S1 > 0] = 1
        dS1 = dS1_relu * dS1_pos
        
        grads['W1'] = X.T.dot(dS1)
        grads['b1'] = dS1.sum(axis=0)
